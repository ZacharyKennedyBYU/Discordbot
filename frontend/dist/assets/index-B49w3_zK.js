(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))e(i);new MutationObserver(i=>{for(const p of i)if(p.type==="childList")for(const d of p.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&e(d)}).observe(document,{childList:!0,subtree:!0});function r(i){const p={};return i.integrity&&(p.integrity=i.integrity),i.referrerPolicy&&(p.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?p.credentials="include":i.crossOrigin==="anonymous"?p.credentials="omit":p.credentials="same-origin",p}function e(i){if(i.ep)return;i.ep=!0;const p=r(i);fetch(i.href,p)}})();const le="/api";async function u(t,a={}){const r=await fetch(`${le}${t}`,{headers:{"Content-Type":"application/json",...a.headers},...a}),e=await r.json();if(!r.ok)throw new Error(e.error||`Request failed (${r.status})`);return e}const fe=async t=>{const a=new FormData;a.append("file",t);const r=await fetch(`${le}/upload`,{method:"POST",body:a}),e=await r.json();if(!r.ok)throw new Error(e.error||`Upload failed (${r.status})`);return e},I={list:()=>u("/providers"),get:t=>u(`/providers/${t}`),create:t=>u("/providers",{method:"POST",body:JSON.stringify(t)}),update:(t,a)=>u(`/providers/${t}`,{method:"PUT",body:JSON.stringify(a)}),delete:t=>u(`/providers/${t}`,{method:"DELETE"}),getModels:t=>u(`/providers/${t}/models`)},y={list:()=>u("/bots"),get:t=>u(`/bots/${t}`),create:t=>u("/bots",{method:"POST",body:JSON.stringify(t)}),update:(t,a)=>u(`/bots/${t}`,{method:"PUT",body:JSON.stringify(a)}),delete:t=>u(`/bots/${t}`,{method:"DELETE"}),start:t=>u(`/bots/${t}/start`,{method:"POST"}),stop:t=>u(`/bots/${t}/stop`,{method:"POST"}),getHistory:t=>u(`/bots/${t}/history`),clearHistory:(t,a)=>u(`/bots/${t}/history${a?`?guild_id=${a}`:""}`,{method:"DELETE"}),getLorebooks:t=>u(`/bots/${t}/lorebooks`),attachLorebook:(t,a)=>u(`/bots/${t}/lorebooks`,{method:"POST",body:JSON.stringify({lorebook_id:a})}),updateLorebook:(t,a,r)=>u(`/bots/${t}/lorebooks/${a}`,{method:"PUT",body:JSON.stringify({overrides:r})}),detachLorebook:(t,a)=>u(`/bots/${t}/lorebooks/${a}`,{method:"DELETE"})},M={list:()=>u("/lorebooks"),get:t=>u(`/lorebooks/${t}`),create:t=>u("/lorebooks",{method:"POST",body:JSON.stringify(t)}),delete:t=>u(`/lorebooks/${t}`,{method:"DELETE"})},se={success:"✓",error:"✕",info:"ℹ"};function m(t,a="info",r=3500){const e=document.getElementById("toast-container"),i=document.createElement("div");i.className=`toast ${a}`,i.innerHTML=`
  ...
  `;const g=document.getElementById("bot-type"),h=document.getElementById("ai-config-card"),B=document.getElementById("prompts-card"),_=document.getElementById("params-card"),G=document.getElementById("false-phrases-card"),Q=document.getElementById("lorebooks-card");let L=[];try{e&&e.false_phrases&&(L=JSON.parse(e.false_phrases||"[]"))}catch{}function X(){const o=g.value==="false";h&&(h.style.display=o?"none":"block"),B&&(B.style.display=o?"none":"block"),_&&(_.style.display=o?"none":"block"),Q&&(Q.style.display=o?"none":"block"),G&&(G.style.display=o?"block":"none")}g.addEventListener("change",X),X();const H=document.getElementById("lb-attach-select"),ve=document.getElementById("lb-attach-btn"),P=document.getElementById("attached-lorebooks");let k={};for(const o of d)try{k[o.lorebook_id]=typeof o.overrides=="string"?JSON.parse(o.overrides||"{}"):o.overrides||{}}catch{k[o.lorebook_id]={}}function j(){H.innerHTML='<option value="">— Select a lorebook to attach —</option>';const o=d.map(l=>l.lorebook_id);p.filter(l=>!o.includes(l.id)).forEach(l=>{const n=document.createElement("option");n.value=l.id,n.textContent=`${l.name} (${l.entry_count||0} entries)`,H.appendChild(n)})}j();function Y(o){return o==="sun"?"☀️":o==="off"?"✖":"🌙"}function be(o){return o==="moon"?"sun":o==="sun"?"off":"moon"}function q(){if(d.length===0){P.innerHTML='<p style="color: var(--text-secondary); text-align: center;">No lorebooks attached.</p>';return}P.innerHTML=d.map(o=>{let s=[];try{const n=typeof o.data=="string"?JSON.parse(o.data):o.data;n.entries&&(s=Object.entries(n.entries))}catch{}const l=k[o.lorebook_id]||{};return`
        <div class="lb-attached-item" data-lbid="${o.lorebook_id}">
          ...
          <div class="lb-entries-list" style="display: none;">
            ${s.map(([n,c])=>{const v=l[n]||"moon",b=c.comment||c.name||`Entry ${n}`,$=(Array.isArray(c.key)?c.key:c.keys||[]).join(", ");return`
      ...
      `}).join(""),P.querySelectorAll(".lb-toggle-expand").forEach(o=>{o.addEventListener("click",()=>{const s=o.closest(".lb-attached-item"),l=s.querySelector(".lb-entries-list"),n=s.querySelector(".lb-expand-arrow"),c=l.style.display!=="none";l.style.display=c?"none":"block",n.textContent=c?"▶":"▼"})}),P.querySelectorAll(".lb-state-btn").forEach(o=>{o.addEventListener("click",()=>{const s=o.dataset.lbid,l=o.dataset.uid,n=o.dataset.state,c=be(n);o.dataset.state=c,o.textContent=Y(c),k[s]||(k[s]={}),k[s][l]=c})}),P.querySelectorAll("[data-detach-lb]").forEach(o=>{o.addEventListener("click",async()=>{const s=parseInt(o.dataset.detachLb),l=d.find(n=>n.lorebook_id===s);if(confirm(`Detach lorebook "${l==null?void 0:l.name}" from this bot?`))try{r||await y.detachLorebook(parseInt(a),s),d=d.filter(n=>n.lorebook_id!==s),delete k[s],q(),j(),m("Lorebook detached","success")}catch(n){m(n.message,"error")}})})}q(),ve.addEventListener("click",async()=>{const o=parseInt(H.value);if(!o)return m("Select a lorebook first","error");try{r||await y.attachLorebook(parseInt(a),o);const s=await M.get(o),l=p.find(n=>n.id===o);d.push({lorebook_id:o,name:(l==null?void 0:l.name)||s.name,data:s.data,overrides:"{}"}),k[o]={},q(),j(),m("Lorebook attached","success")}catch(s){m(s.message,"error")}});const D=document.getElementById("phrases-list"),F=document.getElementById("new-phrase-input"),Z=document.getElementById("add-phrase-btn"),ee=document.getElementById("new-audio-input"),S=document.getElementById("add-audio-btn");function A(){if(D.innerHTML="",L.length===0){D.innerHTML='<li style="padding: 0.75rem; text-align: center; color: var(--text-secondary);">No phrases added yet.</li>';return}L.forEach((o,s)=>{const l=document.createElement("li");l.style.cssText="padding: 0.75rem; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem;";const n=document.createElement("span");n.style.wordBreak="break-word",typeof o=="string"?n.textContent=o:o.type==="audio"&&(n.innerHTML=`🎵 <strong>Audio:</strong> ${E(o.originalName||"response.mp3")}`);const c=document.createElement("button");c.type="button",c.className="btn btn-ghost",c.style.padding="0.25rem 0.5rem",c.style.color="var(--danger)",c.textContent="✖",c.onclick=()=>{L.splice(s,1),A()},l.appendChild(n),l.appendChild(c),D.appendChild(l)})}A(),Z.addEventListener("click",()=>{const o=F.value.trim();o&&(L.push(o),F.value="",A())}),F.addEventListener("keydown",o=>{o.key==="Enter"&&(o.preventDefault(),Z.click())}),S.addEventListener("click",async()=>{const o=ee.files[0];if(!o)return m("Please select a file first","error");try{S.disabled=!0,S.textContent="Uploading...";const s=await fe(o);L.push({type:"audio",path:s.path,originalName:s.originalname}),ee.value="",A(),m("Audio uploaded successfully","success")}catch(s){m(s.message,"error")}finally{S.disabled=!1,S.textContent="Upload MP3"}});const w=document.getElementById("bot-provider"),C=document.getElementById("bot-vision-provider"),U=document.getElementById("bot-model"),te=document.getElementById("use-chat-vision-group"),oe=document.getElementById("use-chat-vision");let O=[];function ye(o){if(!O||O.length===0)return!1;const s=O.find(c=>c.id===o);if(!s||!s.architecture)return!1;s.architecture.instruct_type;const l=s.architecture.modality||"",n=Array.isArray(s.architecture.input_modalities)?s.architecture.input_modalities.join(","):"";return l.includes("image")||n.includes("image")}function J(){const o=U.value.trim();ye(o)?te.style.display="block":(te.style.display="none",oe&&(oe.checked=!1))}U.addEventListener("input",J),U.addEventListener("change",J);async function T(o,s){const l=document.getElementById(s);if(l.innerHTML="",!!o)try{const n=await I.getModels(o);n&&n.data&&Array.isArray(n.data)&&(s==="bot-model-list"&&(O=n.data),n.data.forEach(c=>{const v=document.createElement("option");v.value=c.id;let b=c.name&&c.name!==c.id?c.name:"";s==="bot-model-list"&&(c.architecture&&c.architecture.modality&&c.architecture.modality.includes("image")||c.architecture&&Array.isArray(c.architecture.input_modalities)&&c.architecture.input_modalities.includes("image"))&&(b+=" 👓"),b&&(v.textContent=b),l.appendChild(v)}),s==="bot-model-list"&&J())}catch(n){console.error("Failed to load models:",n)}}w.addEventListener("change",()=>{T(w.value,"bot-model-list"),C.value||T(w.value,"bot-vision-model-list")}),C.addEventListener("change",()=>{T(C.value||w.value,"bot-vision-model-list")}),w.value&&T(w.value,"bot-model-list"),(C.value||w.value)&&T(C.value||w.value,"bot-vision-model-list"),[{input:"bot-temp",display:"temp-val"},{input:"bot-topp",display:"topp-val"},{input:"bot-pp",display:"pp-val"},{input:"bot-fp",display:"fp-val"}].forEach(({input:o,display:s})=>{const l=document.getElementById(o),n=document.getElementById(s);l.addEventListener("input",()=>{n.textContent=l.value})}),document.getElementById("back-btn").addEventListener("click",()=>{window.location.hash="#/"}),document.getElementById("cancel-btn").addEventListener("click",()=>{window.location.hash="#/"});const ae=document.getElementById("clear-history-btn");ae&&ae.addEventListener("click",async()=>{try{const o=await y.getHistory(e.id);if(!o||o.length===0){m("No conversation history to clear","info");return}const s=o.map(v=>`  • ${v.guild_id==="DM"||!v.guild_id?"Direct Messages":`Server ${v.guild_id}`} (${v.message_count} messages)`).join(`
`),l=prompt(`Conversation history for "${e.name}":

${s}

Enter a server ID to clear just that server,
or type "all" to clear everything:`);if(l===null)return;const n=l.trim();if(!n)return;let c;if(n.toLowerCase()==="all"){if(!confirm(`Clear ALL conversation history for "${e.name}" across all servers?`))return;c=await y.clearHistory(e.id)}else{const v=o.find(b=>b.guild_id===n||n.toLowerCase()==="dm"&&(!b.guild_id||b.guild_id==="DM"));if(!v){m("Server ID not found in history","error");return}c=await y.clearHistory(e.id,v.guild_id)}m(`History cleared (${c.messagesRemoved} messages removed)`,"success")}catch(o){m(o.message,"error")}});const re=document.getElementById("delete-btn");re&&re.addEventListener("click",async()=>{if(confirm(`Are you sure you want to delete "${e.name}"? This cannot be undone.`))try{await y.delete(e.id),m(`"${e.name}" deleted`,"info"),window.location.hash="#/"}catch(o){m(o.message,"error")}}),document.getElementById("bot-form").addEventListener("submit",async o=>{var c;o.preventDefault();const s=document.getElementById("save-btn");s.disabled=!0,s.textContent="Saving…";const l={name:document.getElementById("bot-name").value.trim(),model:document.getElementById("bot-model").value.trim(),vision_model:document.getElementById("bot-vision-model").value.trim(),use_chat_vision:((c=document.getElementById("use-chat-vision"))==null?void 0:c.checked)||!1,provider_id:parseInt(document.getElementById("bot-provider").value)||null,vision_provider_id:parseInt(document.getElementById("bot-vision-provider").value)||null,bot_type:document.getElementById("bot-type").value,false_phrases:JSON.stringify(L),system_prompt:document.getElementById("bot-system").value,character_prompt:document.getElementById("bot-character").value,first_message:document.getElementById("bot-first-msg").value,example_messages:document.getElementById("bot-examples").value,prefill:document.getElementById("bot-prefill").value,temperature:parseFloat(document.getElementById("bot-temp").value),top_p:parseFloat(document.getElementById("bot-topp").value),max_tokens:parseInt(document.getElementById("bot-max-tokens").value),max_prompt_tokens:parseInt(document.getElementById("bot-max-prompt").value),presence_penalty:parseFloat(document.getElementById("bot-pp").value),frequency_penalty:parseFloat(document.getElementById("bot-fp").value),auto_start:document.getElementById("bot-autostart").checked},n=document.getElementById("bot-token").value.trim();if(n&&(l.discord_token=n),!l.name){m("Bot name is required","error"),s.disabled=!1,s.textContent=r?"Create Bot":"Save Changes";return}if(r&&!n){m("Discord token is required for new bots","error"),s.disabled=!1,s.textContent="Create Bot";return}try{if(r){const v=await y.create(l);for(const b of d)try{await y.attachLorebook(v.id,b.lorebook_id);const $=k[b.lorebook_id];$&&Object.keys($).length>0&&await y.updateLorebook(v.id,b.lorebook_id,$)}catch($){console.error("Failed to attach lorebook:",$)}m("Bot created!","success")}else{await y.update(e.id,l);for(const v of d){const b=k[v.lorebook_id];if(b)try{await y.updateLorebook(e.id,v.lorebook_id,b)}catch($){console.error("Failed to save lorebook overrides:",$)}}m("Bot updated!","success")}window.location.hash="#/"}catch(v){m(v.message,"error"),s.disabled=!1,s.textContent=r?"Create Bot":"Save Changes"}})}function E(t){if(!t)return"";const a=document.createElement("div");return a.textContent=t,a.innerHTML}function V(t){return t?t.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):""}const ke=Object.freeze(Object.defineProperty({__proto__:null,destroy:Ee,render:ce},Symbol.toStringTag,{value:"Module"}));function $e(){}async function me(t){t.innerHTML=`
    <div class="page">
      <div class="page-header">
        <h1>🔑 API Providers</h1>
  ...
  `,document.getElementById("add-provider-btn").addEventListener("click",()=>W()),K()}async function K(){var a;const t=document.getElementById("provider-list");if(t)try{const r=await I.list();if(r.length===0){t.innerHTML=`
    ...
    `).join(""),t.querySelectorAll("[data-edit-provider]").forEach(e=>{e.addEventListener("click",()=>W(parseInt(e.dataset.editProvider)))}),t.querySelectorAll("[data-delete-provider]").forEach(e=>{e.addEventListener("click",async()=>{const i=parseInt(e.dataset.deleteProvider),p=r.find(d=>d.id===i);if(confirm(`Delete provider "${p==null?void 0:p.name}"? Bots using it will lose their API connection.`))try{await I.delete(i),m("Provider deleted","info"),K()}catch(d){m(d.message,"error")}})})}catch(r){t.innerHTML=`<p style="color: var(--danger);">Failed to load providers: ${r.message}</p>`}}async function W(t=null){var p;const a=t!==null;let r=null;if(a)try{r=await I.get(t)}catch(d){m(d.message,"error");return}(p=document.querySelector(".modal-overlay"))==null||p.remove();const e=document.createElement("div");e.className="modal-overlay",e.innerHTML=`
    <div class="modal">
