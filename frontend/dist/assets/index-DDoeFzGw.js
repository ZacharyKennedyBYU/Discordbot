(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))e(i);new MutationObserver(i=>{for(const p of i)if(p.type==="childList")for(const d of p.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&e(d)}).observe(document,{childList:!0,subtree:!0});function r(i){const p={};return i.integrity&&(p.integrity=i.integrity),i.referrerPolicy&&(p.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?p.credentials="include":i.crossOrigin==="anonymous"?p.credentials="omit":p.credentials="same-origin",p}function e(i){if(i.ep)return;i.ep=!0;const p=r(i);fetch(i.href,p)}})();const de="/api";async function v(t,a={}){const r=await fetch(`${de}${t}`,{headers:{"Content-Type":"application/json",...a.headers},...a}),e=await r.json();if(!r.ok)throw new Error(e.error||`Request failed (${r.status})`);return e}const ge=async t=>{const a=new FormData;a.append("file",t);const r=await fetch(`${de}/upload`,{method:"POST",body:a}),e=await r.json();if(!r.ok)throw new Error(e.error||`Upload failed (${r.status})`);return e},I={list:()=>v("/providers"),get:t=>v(`/providers/${t}`),create:t=>v("/providers",{method:"POST",body:JSON.stringify(t)}),update:(t,a)=>v(`/providers/${t}`,{method:"PUT",body:JSON.stringify(a)}),delete:t=>v(`/providers/${t}`,{method:"DELETE"}),getModels:t=>v(`/providers/${t}/models`)},f={list:()=>v("/bots"),get:t=>v(`/bots/${t}`),create:t=>v("/bots",{method:"POST",body:JSON.stringify(t)}),update:(t,a)=>v(`/bots/${t}`,{method:"PUT",body:JSON.stringify(a)}),delete:t=>v(`/bots/${t}`,{method:"DELETE"}),start:t=>v(`/bots/${t}/start`,{method:"POST"}),stop:t=>v(`/bots/${t}/stop`,{method:"POST"}),getHistory:t=>v(`/bots/${t}/history`),clearHistory:(t,a)=>v(`/bots/${t}/history${a?`?guild_id=${a}`:""}`,{method:"DELETE"}),getLorebooks:t=>v(`/bots/${t}/lorebooks`),attachLorebook:(t,a)=>v(`/bots/${t}/lorebooks`,{method:"POST",body:JSON.stringify({lorebook_id:a})}),updateLorebook:(t,a,r)=>v(`/bots/${t}/lorebooks/${a}`,{method:"PUT",body:JSON.stringify({overrides:r})}),detachLorebook:(t,a)=>v(`/bots/${t}/lorebooks/${a}`,{method:"DELETE"})},A={list:()=>v("/lorebooks"),get:t=>v(`/lorebooks/${t}`),create:t=>v("/lorebooks",{method:"POST",body:JSON.stringify(t)}),delete:t=>v(`/lorebooks/${t}`,{method:"DELETE"})},ne={success:"✓",error:"✕",info:"ℹ"};function m(t,a="info",r=3500){const e=document.getElementById("toast-container"),i=document.createElement("div");i.className=`toast ${a}`,i.innerHTML=`
        ...
        <div class="card" style="margin-bottom: 1.5rem;">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input class="form-input" id="bot-name" type="text" placeholder="My Cool Bot" value="${N(e==null?void 0:e.name)}" required />
            </div>
            <div class="form-group">
          ...
          <div class="form-group" style="margin-top: 1rem;">
            ...
            <input class="form-input" id="bot-allowed-guilds" type="text" placeholder="1234567890, 0987654321" value="${N(g(e==null?void 0:e.allowed_guilds))}" />
          </div>
        </div>
        ...
        <div class="card" id="ai-config-card" style="margin-bottom: 1.5rem;">
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input class="form-input" id="bot-model" type="text" list="bot-model-list" placeholder="deepseek/deepseek-v3.2" value="${N((e==null?void 0:e.model)||"deepseek/deepseek-v3.2")}" autocomplete="off" />
            ...
            <div class="form-group">
              ...
              <input class="form-input" id="bot-vision-model" type="text" list="bot-vision-model-list" placeholder="openai/gpt-4o-mini" value="${N((e==null?void 0:e.vision_model)||"")}" autocomplete="off" />
  ...
  `;const h=document.getElementById("bot-type"),_=document.getElementById("ai-config-card"),B=document.getElementById("prompts-card"),K=document.getElementById("params-card"),Q=document.getElementById("false-phrases-card"),X=document.getElementById("lorebooks-card");let L=[];try{e&&e.false_phrases&&(L=JSON.parse(e.false_phrases||"[]"))}catch{}function Y(){const o=h.value==="false";_&&(_.style.display=o?"none":"block"),B&&(B.style.display=o?"none":"block"),K&&(K.style.display=o?"none":"block"),X&&(X.style.display=o?"none":"block"),Q&&(Q.style.display=o?"block":"none")}h.addEventListener("change",Y),Y();const j=document.getElementById("lb-attach-select"),be=document.getElementById("lb-attach-btn"),S=document.getElementById("attached-lorebooks");let k={};for(const o of d)try{k[o.lorebook_id]=typeof o.overrides=="string"?JSON.parse(o.overrides||"{}"):o.overrides||{}}catch{k[o.lorebook_id]={}}function D(){j.innerHTML='<option value="">— Select a lorebook to attach —</option>';const o=d.map(l=>l.lorebook_id);p.filter(l=>!o.includes(l.id)).forEach(l=>{const n=document.createElement("option");n.value=l.id,n.textContent=`${l.name} (${l.entry_count||0} entries)`,j.appendChild(n)})}D();function Z(o){return o==="sun"?"☀️":o==="off"?"✖":"🌙"}function fe(o){return o==="moon"?"sun":o==="sun"?"off":"moon"}function q(){if(d.length===0){S.innerHTML='<p style="color: var(--text-secondary); text-align: center;">No lorebooks attached.</p>';return}S.innerHTML=d.map(o=>{let s=[];try{const n=typeof o.data=="string"?JSON.parse(o.data):o.data;n.entries&&(s=Object.entries(n.entries))}catch{}const l=k[o.lorebook_id]||{};return`
      ...
      `}).join(""),S.querySelectorAll(".lb-toggle-expand").forEach(o=>{o.addEventListener("click",()=>{const s=o.closest(".lb-attached-item"),l=s.querySelector(".lb-entries-list"),n=s.querySelector(".lb-expand-arrow"),c=l.style.display!=="none";l.style.display=c?"none":"block",n.textContent=c?"▶":"▼"})}),S.querySelectorAll(".lb-state-btn").forEach(o=>{o.addEventListener("click",()=>{const s=o.dataset.lbid,l=o.dataset.uid,n=o.dataset.state,c=fe(n);o.dataset.state=c,o.textContent=Z(c),k[s]||(k[s]={}),k[s][l]=c})}),S.querySelectorAll("[data-detach-lb]").forEach(o=>{o.addEventListener("click",async()=>{const s=parseInt(o.dataset.detachLb),l=d.find(n=>n.lorebook_id===s);if(confirm(`Detach lorebook "${l==null?void 0:l.name}" from this bot?`))try{r||await f.detachLorebook(parseInt(a),s),d=d.filter(n=>n.lorebook_id!==s),delete k[s],q(),D(),m("Lorebook detached","success")}catch(n){m(n.message,"error")}})})}q(),be.addEventListener("click",async()=>{const o=parseInt(j.value);if(!o)return m("Select a lorebook first","error");try{r||await f.attachLorebook(parseInt(a),o);const s=await A.get(o),l=p.find(n=>n.id===o);d.push({lorebook_id:o,name:(l==null?void 0:l.name)||s.name,data:s.data,overrides:"{}"}),k[o]={},q(),D(),m("Lorebook attached","success")}catch(s){m(s.message,"error")}});const F=document.getElementById("phrases-list"),U=document.getElementById("new-phrase-input"),ee=document.getElementById("add-phrase-btn"),te=document.getElementById("new-audio-input"),C=document.getElementById("add-audio-btn");function M(){if(F.innerHTML="",L.length===0){F.innerHTML='<li style="padding: 0.75rem; text-align: center; color: var(--text-secondary);">No phrases added yet.</li>';return}L.forEach((o,s)=>{const l=document.createElement("li");l.style.cssText="padding: 0.75rem; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem;";const n=document.createElement("span");n.style.wordBreak="break-word",typeof o=="string"?n.textContent=o:o.type==="audio"&&(n.innerHTML=`🎵 <strong>Audio:</strong> ${E(o.originalName||"response.mp3")}`);const c=document.createElement("button");c.type="button",c.className="btn btn-ghost",c.style.padding="0.25rem 0.5rem",c.style.color="var(--danger)",c.textContent="✖",c.onclick=()=>{L.splice(s,1),M()},l.appendChild(n),l.appendChild(c),F.appendChild(l)})}M(),ee.addEventListener("click",()=>{const o=U.value.trim();o&&(L.push(o),U.value="",M())}),U.addEventListener("keydown",o=>{o.key==="Enter"&&(o.preventDefault(),ee.click())}),C.addEventListener("click",async()=>{const o=te.files[0];if(!o)return m("Please select a file first","error");try{C.disabled=!0,C.textContent="Uploading...";const s=await ge(o);L.push({type:"audio",path:s.path,originalName:s.originalname}),te.value="",M(),m("Audio uploaded successfully","success")}catch(s){m(s.message,"error")}finally{C.disabled=!1,C.textContent="Upload MP3"}});const w=document.getElementById("bot-provider"),P=document.getElementById("bot-vision-provider"),J=document.getElementById("bot-model"),oe=document.getElementById("use-chat-vision-group"),ae=document.getElementById("use-chat-vision");let O=[];function ye(o){if(!O||O.length===0)return!1;const s=O.find(c=>c.id===o);if(!s||!s.architecture)return!1;s.architecture.instruct_type;const l=s.architecture.modality||"",n=Array.isArray(s.architecture.input_modalities)?s.architecture.input_modalities.join(","):"";return l.includes("image")||n.includes("image")}function z(){const o=J.value.trim();ye(o)?oe.style.display="block":(oe.style.display="none",ae&&(ae.checked=!1))}J.addEventListener("input",z),J.addEventListener("change",z);async function T(o,s){const l=document.getElementById(s);if(l.innerHTML="",!!o)try{const n=await I.getModels(o);n&&n.data&&Array.isArray(n.data)&&(s==="bot-model-list"&&(O=n.data),n.data.forEach(c=>{const u=document.createElement("option");u.value=c.id;let b=c.name&&c.name!==c.id?c.name:"";s==="bot-model-list"&&(c.architecture&&c.architecture.modality&&c.architecture.modality.includes("image")||c.architecture&&Array.isArray(c.architecture.input_modalities)&&c.architecture.input_modalities.includes("image"))&&(b+=" 👓"),b&&(u.textContent=b),l.appendChild(u)}),s==="bot-model-list"&&z())}catch(n){console.error("Failed to load models:",n)}}w.addEventListener("change",()=>{T(w.value,"bot-model-list"),P.value||T(w.value,"bot-vision-model-list")}),P.addEventListener("change",()=>{T(P.value||w.value,"bot-vision-model-list")}),w.value&&T(w.value,"bot-model-list"),(P.value||w.value)&&T(P.value||w.value,"bot-vision-model-list"),[{input:"bot-temp",display:"temp-val"},{input:"bot-topp",display:"topp-val"},{input:"bot-pp",display:"pp-val"},{input:"bot-fp",display:"fp-val"}].forEach(({input:o,display:s})=>{const l=document.getElementById(o),n=document.getElementById(s);l.addEventListener("input",()=>{n.textContent=l.value})}),document.getElementById("back-btn").addEventListener("click",()=>{window.location.hash="#/"}),document.getElementById("cancel-btn").addEventListener("click",()=>{window.location.hash="#/"});const re=document.getElementById("clear-history-btn");re&&re.addEventListener("click",async()=>{try{const o=await f.getHistory(e.id);if(!o||o.length===0){m("No conversation history to clear","info");return}const s=o.map(u=>`  • ${u.guild_id==="DM"||!u.guild_id?"Direct Messages":`Server ${u.guild_id}`} (${u.message_count} messages)`).join(`
`),l=prompt(`Conversation history for "${e.name}":

${s}

Enter a server ID to clear just that server,
or type "all" to clear everything:`);if(l===null)return;const n=l.trim();if(!n)return;let c;if(n.toLowerCase()==="all"){if(!confirm(`Clear ALL conversation history for "${e.name}" across all servers?`))return;c=await f.clearHistory(e.id)}else{const u=o.find(b=>b.guild_id===n||n.toLowerCase()==="dm"&&(!b.guild_id||b.guild_id==="DM"));if(!u){m("Server ID not found in history","error");return}c=await f.clearHistory(e.id,u.guild_id)}m(`History cleared (${c.messagesRemoved} messages removed)`,"success")}catch(o){m(o.message,"error")}});const se=document.getElementById("delete-btn");se&&se.addEventListener("click",async()=>{if(confirm(`Are you sure you want to delete "${e.name}"? This cannot be undone.`))try{await f.delete(e.id),m(`"${e.name}" deleted`,"info"),window.location.hash="#/"}catch(o){m(o.message,"error")}}),document.getElementById("bot-form").addEventListener("submit",async o=>{var c;o.preventDefault();const s=document.getElementById("save-btn");s.disabled=!0,s.textContent="Saving…";const l={name:document.getElementById("bot-name").value.trim(),model:document.getElementById("bot-model").value.trim(),vision_model:document.getElementById("bot-vision-model").value.trim(),use_chat_vision:((c=document.getElementById("use-chat-vision"))==null?void 0:c.checked)||!1,provider_id:parseInt(document.getElementById("bot-provider").value)||null,vision_provider_id:parseInt(document.getElementById("bot-vision-provider").value)||null,bot_type:document.getElementById("bot-type").value,false_phrases:JSON.stringify(L),system_prompt:document.getElementById("bot-system").value,character_prompt:document.getElementById("bot-character").value,first_message:document.getElementById("bot-first-msg").value,example_messages:document.getElementById("bot-examples").value,prefill:document.getElementById("bot-prefill").value,temperature:parseFloat(document.getElementById("bot-temp").value),top_p:parseFloat(document.getElementById("bot-topp").value),max_tokens:parseInt(document.getElementById("bot-max-tokens").value),max_prompt_tokens:parseInt(document.getElementById("bot-max-prompt").value),presence_penalty:parseFloat(document.getElementById("bot-pp").value),frequency_penalty:parseFloat(document.getElementById("bot-fp").value),auto_start:document.getElementById("bot-autostart").checked,allowed_guilds:JSON.stringify(document.getElementById("bot-allowed-guilds").value.split(",").map(u=>u.trim()).filter(u=>u))},n=document.getElementById("bot-token").value.trim();if(n&&(l.discord_token=n),!l.name){m("Bot name is required","error"),s.disabled=!1,s.textContent=r?"Create Bot":"Save Changes";return}if(r&&!n){m("Discord token is required for new bots","error"),s.disabled=!1,s.textContent="Create Bot";return}try{if(r){const u=await f.create(l);for(const b of d)try{await f.attachLorebook(u.id,b.lorebook_id);const $=k[b.lorebook_id];$&&Object.keys($).length>0&&await f.updateLorebook(u.id,b.lorebook_id,$)}catch($){console.error("Failed to attach lorebook:",$)}m("Bot created!","success")}else{await f.update(e.id,l);for(const u of d){const b=k[u.lorebook_id];if(b)try{await f.updateLorebook(e.id,u.lorebook_id,b)}catch($){console.error("Failed to save lorebook overrides:",$)}}m("Bot updated!","success")}window.location.hash="#/"}catch(u){m(u.message,"error"),s.disabled=!1,s.textContent=r?"Create Bot":"Save Changes"}})}function E(t){if(!t)return"";const a=document.createElement("div");return a.textContent=t,a.innerHTML}function N(t){return t?t.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):""}const $e=Object.freeze(Object.defineProperty({__proto__:null,destroy:ke,render:me},Symbol.toStringTag,{value:"Module"}));function we(){}async function pe(t){t.innerHTML=`
    <div class="page">
      <div class="page-header">
        <h1>🔑 API Providers</h1>
  ...
  `,document.getElementById("add-provider-btn").addEventListener("click",()=>W()),G()}async function G(){var a;const t=document.getElementById("provider-list");if(t)try{const r=await I.list();if(r.length===0){t.innerHTML=`
        <div class="empty-state" style="grid-column: 1 / -1;">
          <div class="empty-icon">🔑</div>
    ...
    `).join(""),t.querySelectorAll("[data-edit-provider]").forEach(e=>{e.addEventListener("click",()=>W(parseInt(e.dataset.editProvider)))}),t.querySelectorAll("[data-delete-provider]").forEach(e=>{e.addEventListener("click",async()=>{const i=parseInt(e.dataset.deleteProvider),p=r.find(d=>d.id===i);if(confirm(`Delete provider "${p==null?void 0:p.name}"? Bots using it will lose their API connection.`))try{await I.delete(i),m("Provider deleted","info"),G()}catch(d){m(d.message,"error")}})})}catch(r){t.innerHTML=`<p style="color: var(--danger);">Failed to load providers: ${r.message}</p>`}}async function W(t=null){var p;const a=t!==null;let r=null;if(a)try{r=await I.get(t)}catch(d){m(d.message,"error");return}(p=document.querySelector(".modal-overlay"))==null||p.remove();const e=document.createElement("div");e.className="modal-overlay",e.innerHTML=`
    <div class="modal">
  ...
  `,document.body.appendChild(e);const i=()=>e.remove();document.getElementById("modal-close").addEventListener("click",i),document.getElementById("modal-cancel").addEventListener("click",i),e.addEventListener("click",d=>{d.target===e&&i()}),document.getElementById("provider-form").addEventListener("submit",async d=>{d.preventDefault();const y={name:document.getElementById("prov-name").value.trim(),base_url:document.getElementById("prov-url").value.trim()||"https://openrouter.ai/api/v1"},g=document.getElementById("prov-key").value.trim();if(g&&(y.api_key=g),!y.name){m("Name is required","error");return}if(!a&&!g){m("API key is required","error");return}try{a?(await I.update(t,y),m("Provider updated!","success")):(await I.create(y),m("Provider created!","success")),i(),G()}catch(h){m(h.message,"error")}})}function ie(t){if(!t)return"";const a=document.createElement("div");return a.textContent=t,a.innerHTML}function le(t){return t?t.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):""}const xe=Object.freeze(Object.defineProperty({__proto__:null,destroy:we,render:pe},Symbol.toStringTag,{value:"Module"}));function Le(){}async function ue(t){let a=[];try{a=await A.list()}catch(d){t.innerHTML=`<div class="page"><p style="color:var(--danger)">Error loading lorebooks: ${d.message}</p></div>`;return}t.innerHTML=`
    <div class="page" style="max-width: 760px;">
      <div class="page-header">
        <h1>📚 Lorebooks</h1>
      </div>
  ...
  `;const r=document.getElementById("lb-list"),e=document.getElementById("lb-file"),i=document.getElementById("upload-lb-btn");function p(){if(a.length===0){r.innerHTML='<p style="color: var(--text-secondary); text-align: center;">No lorebooks uploaded yet.</p>';return}r.innerHTML=a.map(d=>`
    ...
    `).join(""),r.querySelectorAll("[data-delete-lb]").forEach(d=>{d.addEventListener("click",async()=>{const y=parseInt(d.dataset.deleteLb),g=a.find(h=>h.id===y);if(confirm(`Delete lorebook "${g==null?void 0:g.name}"? This will also remove it from all bots.`))try{await A.delete(y),a=a.filter(h=>h.id!==y),p(),m("Lorebook deleted","success")}catch(h){m(h.message,"error")}})})}p(),i.addEventListener("click",async()=>{const d=e.files[0];if(!d)return m("Please select a JSON file first","error");try{i.disabled=!0,i.textContent="Uploading…";const y=await d.text(),g=JSON.parse(y);if(!g.entries)throw new Error('File must contain an "entries" object');const h=g.name||d.name.replace(/\.json$/i,""),_=await A.create({name:h,data:y}),B=Object.keys(g.entries).length;a.push({id:_.id,name:_.name,entry_count:B}),p(),e.value="",m(`Lorebook "${_.name}" uploaded (${B} entries)`,"success")}catch(y){m(y.message,"error")}finally{i.disabled=!1,i.textContent="📤 Upload"}})}function Ie(t){if(!t)return"";const a=document.createElement("div");return a.textContent=t,a.innerHTML}const _e=Object.freeze(Object.defineProperty({__proto__:null,destroy:Le,render:ue},Symbol.toStringTag,{value:"Module"})),Be=document.getElementById("app");let x=null;function Se(t){return`
    <nav class="navbar">
  ...
  `}async function ve(){x!=null&&x.destroy&&x.destroy();const t=window.location.hash||"#/";let a="dashboard";t.startsWith("#/providers")?a="providers":t.startsWith("#/lorebooks")?a="lorebooks":t.startsWith("#/bots/")&&(a="botEditor"),Be.innerHTML=Se(a)+'<div id="page-content"></div>';const r=document.getElementById("page-content");if(a==="providers")x=xe,await pe(r);else if(a==="lorebooks")x=_e,await ue(r);else if(a==="botEditor"){const e=t.split("/").pop();x=$e,await me(r,e)}else x=Ee,ce(r)}window.addEventListener("hashchange",ve);window.addEventListener("DOMContentLoaded",()=>{window.location.hash||(window.location.hash="#/"),ve()});
